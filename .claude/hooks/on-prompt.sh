...
# [최적화] 기본 모드 1회 blocking 쿼리, 경고 모드 INSERT non-blocking
...
if [ "$CTX_ALERT" = "compacted" ]; then
    ...
    if [ -n "$CTX_RESTORED_AT" ] && [ "$CTX_RESTORED_AT" = "$CTX_UPDATED" ]; then
        ...
        # 기본 모드: SESSION_ID + 편집수 + 태스크 수를 1회 조회
        read SESSION_ID SESSION_EDITS PENDING_TASKS <<< $(sqlite3 "$DB_PATH" "
            SELECT
                (SELECT id FROM sessions ORDER BY id DESC LIMIT 1),
                (SELECT COUNT(DISTINCT file_path) FROM tool_usage WHERE session_id=(SELECT id FROM sessions ORDER BY id DESC LIMIT 1)),
    ...
    # === 기본 모드 (최소 주입) — 1회 blocking으로 전부 조회 ===
    read SESSION_ID SESSION_EDITS PENDING_TASKS <<< $(sqlite3 "$DB_PATH" "
        SELECT
            (SELECT id FROM sessions ORDER BY id DESC LIMIT 1),
            (SELECT COUNT(DISTINCT file_path) FROM tool_usage WHERE session_id=(SELECT id FROM sessions ORDER BY id DESC LIMIT 1)),
