...
| 에이전트 | 역할 | 모델 | 코드 수정 |
|----------|------|:----:|:---------:|
| **ralph** | 끈질긴 구현 — 빌드+테스트 통과까지 절대 멈추지 않음 | Sonnet | 가능 |
| **planner** | 요청 분석 → 태스크 분해 + 수용 기준 정의 | Opus | 불가 |
| **architect** | 설계 및 아키텍처 타당성 검토 | Opus | 불가 |
| **verifier** | 빌드/테스트/타입체크 결과 기반 검증 | Haiku | 불가 |
| **reviewer** | 코드 리뷰 — 2단계 프로세스(발견→필터링)로 이슈 누락 방지 | Opus | 불가 |
| **debugger** | 버그/에러 근본 원인 진단 — 가설 집중 검증 | Opus | 불가 |
| **test-engineer** | 테스트 전략 수립 + 테스트 코드 작성 | Sonnet | 가능 |

**모델 배정 원칙**:
- **Opus** — 깊은 추론이 핵심인 역할 (계획, 설계 검토, 코드 리뷰, 근본 원인 진단)
- **Sonnet** — 도구 호출 빈도가 높은 실행 역할 (코드 구현, 테스트 작성)
- **Haiku** — 단순 실행+판정 역할 (빌드/테스트 검증)
