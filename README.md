...
OMC(oh-my-claudecode) 종속 없이 **Claude Code 네이티브 기능만**으로 동작합니다.
커맨드 하나(`/project-init`)로 에이전트, 훅, DB, HUD가 세팅됩니다.
...
| 프로젝트마다 Claude Code 설정을 처음부터 구성 | `/project-init` 한 번으로 전체 환경 세팅 |
...
> **Ralph란?** OMC의 Ralph에서 영감을 받은 "포기하지 않는" 에이전트입니다.
...
| 세션/주간 리밋 | [Anthropic OAuth API](https://api.anthropic.com/api/oauth/usage) | 사용률 % + 리셋 잔여 시간 |
...
│   ├── commands/                      # 글로벌 명령어
│   │   ├── project-init.md            #   /project-init (새 프로젝트)
│   │   └── project-migration.md       #   /project-migration (기존 프로젝트)
│   ├── scripts/
...
| 원칙 | 설명 |
|------|------|
| **OMC 비종속** | Claude Code 네이티브 기능만 사용 (Agent, Hook, Command) |
...
- [oh-my-claudecode](https://github.com/Yeachan-Heo/oh-my-claudecode) — Ralph 에이전트, HUD 등 영감을 받은 프로젝트
