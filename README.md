...
```
post-tool-edit.sh ──┐
post-tool-bash.sh ──┼──▶ .claude/.hook_feedback (append)
on-stop.sh ─────────┘
                          ↓ 다음 프롬프트 시
on-prompt.sh ◀──── .hook_feedback 읽기 → stdout 출력 → 파일 삭제
```
...
```
[CC#1.0.80] | ~/work/myproject | 5h:39%(2h37m) wk:15%(4d7h) | Opus | ctx:14% | agents:3/19
 ─────────    ────────────────   ──────────────────────────   ────   ───────   ──────────
  CC 버전          CWD           세션 리밋     주간 리밋      모델   컨텍스트%  활성/전체 에이전트
```
...
| agents | 서브에이전트 transcript | 활성/전체 에이전트 수 (활성 시 노란색 강조) |
